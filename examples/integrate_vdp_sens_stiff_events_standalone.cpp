...
struct observer {
  std::vector<AD>& times;
  std::vector<AD>& y; // x, v, x, v, ...
  int& event_count;
  explicit observer(std::vector<AD>& t, std::vector<AD>& y_, int& ec)
    : times(t), y(y_), event_count(ec) {}
  void operator()(vector<AD>& x, const AD& t) {
    const double curr_t = CppAD::Value(t);
    if (curr_t == 5.0 && event_count < 1) { x[0] = 2.0; event_count++; }
    times.push_back(t);
    y.push_back(x[0]);
    y.push_back(x[1]);
  }
