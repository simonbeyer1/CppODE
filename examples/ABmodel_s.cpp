...
struct ode_system {
  ...
  void operator()(const vector<AD>& x, vector<AD>& dxdt, const AD& t) {
    dxdt[0] = -CppAD::pow(x[0],2)*params[2]*t;
    dxdt[1] = x[0]*params[2]-x[1]*params[3];
  }
};
...
struct jacobian {
  ...
  void operator()(const vector<AD>& x, matrix<AD>& J, const AD& t, vector<AD>& dfdt) {
    ...
    J(1,0)=params[2];
    J(1,1)=-params[3];
    dfdt[0]=-CppAD::pow(x[0],2)*params[2];
    dfdt[1]=0;
  }
};
...
struct observer {
  ...
  void operator()(vector<AD>& x, const AD& t) {
    if (CppAD::Value(t) == 5) apply_event(x, 0, 2, 1);
    times.push_back(t);
...
extern "C" SEXP solve(SEXP timesSEXP, SEXP paramsSEXP, SEXP abstolSEXP, SEXP reltolSEXP) {
  ...
  SET_STRING_ELT(coln, 2, Rf_mkChar("B"));
  SET_STRING_ELT(coln, 3, Rf_mkChar("A.A"));
  SET_STRING_ELT(coln, 4, Rf_mkChar("A.B"));
  SET_STRING_ELT(coln, 5, Rf_mkChar("A.k1"));
  SET_STRING_ELT(coln, 6, Rf_mkChar("A.k2"));
  SET_STRING_ELT(coln, 7, Rf_mkChar("B.A"));
  SET_STRING_ELT(coln, 8, Rf_mkChar("B.B"));
  SET_STRING_ELT(coln, 9, Rf_mkChar("B.k1"));
  SET_STRING_ELT(coln, 10, Rf_mkChar("B.k2"));
