...
extract_long <- function(res, system_label) {
  ...
  states <- dimnames(res$sens1)[[1]]  # e.g. c("x", "v")
  pars   <- dimnames(res$sens1)[[2]]  # e.g. c("x", "v", "omega0", "gamma")
  nt     <- length(res$time)

  # — States —
  vars_wide <- as.data.table(cbind(time = res$time, t(res$variable)))
  dt_states <- melt(vars_wide, id.vars = "time", variable.name = "name", value.name = "value")
  dt_states[, type := "state"]

  # — First-order sensitivities: sens1[state, par, time] —
  sens <- res$sens1
  ...
  # — Second-order sensitivities: sens2[state, par, par, time] —
  sens2 <- res$sens2
  np <- length(pars)
