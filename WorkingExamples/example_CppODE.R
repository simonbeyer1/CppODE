...
model_c <- CppODE(eqns, events = events, deriv2 = F, outdir = getwd(), modelname = "Amodel_c", compile = T, useDenseOutput = F)
model_d <- CppODE(eqns, events = events, deriv2 = F, outdir = getwd(), modelname = "Amodel_d", compile = T, useDenseOutput = T)
...
system.time({solveODE(model_c, times, params, maxroot = 1)})
system.time({solveODE(model_d, times, params, maxroot = 1)})
