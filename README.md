...
shared libraries on compute nodes without `LD_LIBRARY_PATH`, which
matters for schedulers (SLURM, PBS, …) that strip the user environment
between login and worker shells.
...
- The Nordsieck multistep stepper (BDF/NDF and Adams-Moulton) in
  ...
  license.
- The Rosenbrock4 stepper architecture in `cppode_rosenbrock4.hpp` (and
