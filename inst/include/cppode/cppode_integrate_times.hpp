...
namespace cppode {
  namespace detail {
    ...
    // Stepper has current_method_order() — use it (covers all instrumented types)
    ...
    // Detect restart_from_order1(x, t, dt) — BDF-specific reset
    template<class S, class State, class Time, class = void>
    ...
    // Detect reset_after_event(dt) — Rosenbrock-style reset (invalidate LU, reset PI state)
    ...
    inline void reset_stepper_unified(S& st, State& x, Time t, Time& dt) {
      if constexpr (::cppode::needs_restart_after_event_v<S>) {
        // Multistep (BDF): discard history, restart at order 1
    ...
    class EventEngine {
      ...
      // Profiler access — resolves through dense_output → controller chain
      ...
      //
      // Multistep methods (BDF, Adams, ...):
      //   Full restart: re-estimate dt from the post-event state using
      //   the Hairer-Norsett-Wanner heuristic (if a DtEstimator is
      //   provided), then build a fresh Nordsieck array at order 1.
      //   This avoids inheriting a dt that was tuned to pre-event
      //   dynamics, which would cause many rejected steps.
      //
      // Single-step methods (Rosenbrock, ...): initialize()
      //   → sufficient because no history to invalidate.
      // ----------------------------------------------------------------
      void init_stepper_after_event(State& x, Time t, Time& dt) {
        if constexpr (::cppode::needs_restart_after_event_v<Stepper>) {
          // Re-estimate dt from scratch based on post-event state
          if constexpr (!std::is_same_v<DtEstimator, no_dt_estimator>) {
            dt = m_dt_estimator(x, t);
          }
          // 1. Set dense output wrapper state buffers and times
          m_st.initialize(x, t, dt);
          // 2. Evaluate f(t, x) for Nordsieck initialization
          State f0(x.size());
          m_sys.first(x, f0, t);
          // 3. Full inner stepper init: zn[0]=x, zn[1]=h·f, order=1
          m_st.controlled_stepper().stepper().initialize(x, t, f0, dt);
          // 4. Reset controller state (error history, LU cache)
      ...
      {
        // Profiling wrapper around observer
