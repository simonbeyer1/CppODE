...
public:
  ...
  // ====================================================================
  //  jacobian_one_norm — return ||J||_1 = max_j sum_i |J[i,j]|
  //
  //  Used by the LSODA-style stiffness detector (cppode_multistepper_
  //  stiffness_detector.hpp) to estimate the dominant Lipschitz
  //  constant.  Returns 0 if no Jacobian is currently available.
  //
  //  The Jacobian is stored pre-negated (the codegen writes −J), but
  //  ||−J||_1 = ||J||_1 since the norm sums absolute values, so we
  //  read from the same storage without un-negating.
  //
  //  Cost:
  //    Sparse:  O(nnz)   — single pass over Ax
  //    Dense:   O(n^2)   — two nested loops; column-major access
  //                        means each column is contiguous in memory
  //
  //  AD-aware: uses ad_lu::scalar_value() to extract the value
  //  component when the entries are FADBAD F<...> types.  The norm
  //  is a real number; we don't propagate sensitivities through it.
  // ====================================================================

  double jacobian_one_norm() const
  {
    if (!m_has_valid_jac) return 0.0;

    double max_col_sum = 0.0;

    if constexpr (is_sparse) {
      // CSC: column j occupies Ax[Ap[j] .. Ap[j+1]-1].
      const int n_cols = m_W_sparse.n;
      for (int j = 0; j < n_cols; ++j) {
        double col_sum = 0.0;
        const int p_start = m_W_sparse.Ap[j];
        const int p_end   = m_W_sparse.Ap[j + 1];
        for (int p = p_start; p < p_end; ++p) {
          col_sum += std::abs(
            ad_lu::scalar_value(m_W_sparse.Ax[p]));
        }
        if (col_sum > max_col_sum) max_col_sum = col_sum;
      }
    } else {
      // Dense column-major: column j is contiguous starting at
      // m_W_temp.data[j * n_rows], with n_rows entries.
      // We use m_jac_cache (which holds the most recently cached
      // Jacobian) when available, falling back to m_W_temp.  The
      // cache and m_W_temp are byte-equal right after cache_jacobian,
      // but m_W_temp may have been overwritten by a later W-build,
      // whereas m_jac_cache survives.
      const auto& src =
        (m_jac_cache.m_v.rows() > 0) ? m_jac_cache.m_v : m_W_temp;
      const int n_rows = src.rows();
      const int n_cols = src.cols();
      for (int j = 0; j < n_cols; ++j) {
        double col_sum = 0.0;
        const auto* col_ptr = src.data.data() + j * n_rows;
        for (int i = 0; i < n_rows; ++i) {
          col_sum += std::abs(
            ad_lu::scalar_value(col_ptr[i]));
        }
        if (col_sum > max_col_sum) max_col_sum = col_sum;
      }
    }
