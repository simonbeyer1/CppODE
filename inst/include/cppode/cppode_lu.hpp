  ...
  private:
    ...
    void factorize_W_dense(size_t n, value_type inv_gamma_dt)
    {
      ...
      if constexpr (!ad_lu::is_ad<value_type>::value) {
        // Fused negate-copy: W = -J  (single pass, no separate dcopy+dscal)
        const double* __restrict__ src = m_jac.m_v.data.data();
        double* __restrict__ dst = m_W_temp.data.data();
        for (size_t k = 0; k < nn; ++k)
          dst[k] = -src[k];
        for (size_t i = 0; i < n; ++i)
          m_W_temp(i, i) += inv_gamma_dt;

        // Move W_temp's data into LU solver (O(1) swap, no memcpy).
        // After swap, m_W_temp.data holds the old m_lu_data buffer.
        // Ensure it has the right size for the next call (no-op after
        // the first factorization since both vectors are n*n).
        m_dense_lu.factorize_move(m_W_temp);
        m_W_temp.data.resize(nn);
      } else {
        // AD path: use factorize (copy) — IFT needs the original W matrix
        // and the recursive AD structure makes move semantics fragile.
        for (size_t k = 0; k < nn; ++k)
    ...
    // ====================================================================
    //  Dense refactorize from cache: W = inv_gamma_dt · I − J_cache
    //
    //  Like factorize_W_dense but reads from m_jac_cache instead of m_jac.
    //  Eliminates the 1.28 MB copy m_jac_cache → m_jac that the old code did.
    // ====================================================================
    ...
    void refactorize_W_from_cache_dense(size_t n, value_type inv_gamma_dt)
    {
      ...
      if constexpr (!ad_lu::is_ad<value_type>::value) {
        const double* __restrict__ src = m_jac_cache.m_v.data.data();
        double* __restrict__ dst = m_W_temp.data.data();
        for (size_t k = 0; k < nn; ++k)
          dst[k] = -src[k];
        for (size_t i = 0; i < n; ++i)
        ...
        for (size_t k = 0; k < nn; ++k)
          m_W_temp.data[k] = -m_jac_cache.m_v.data[k];
        for (size_t i = 0; i < n; ++i)
          m_W_temp(i, i) += inv_gamma_dt;
